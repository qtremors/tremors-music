...
export function formatTime(inputSeconds: number | null | undefined): string {
  if (!inputSeconds && inputSeconds !== 0) return "0:00"; // Null/undefined check
  const seconds = Number(inputSeconds); // Explicit cast
  if (isNaN(seconds) || seconds === Infinity || seconds < 0) return "0:00";

  // Cap at 24 hours to prevent massive overflow display abuse
